...
-- Order of board statuses (slot 1..6) - canonical ordering (visual still compact first-in)
local STATUS_ORDER = {
...
local function safePlace(obj, worldPos)
...
-- =========================
local function buildOrderedStatusTokens(color)
  local s = ensureStatuses(color)
  local out = {}
  for _, tag in ipairs(STATUS_ORDER) do
    local tok = s.active[tag]
    if tok then
      -- ADDICTION can be multiple tokens (array), others are single token
      if tag == TAG_STATUS_ADDICTION and type(tok) == "table" and #tok > 0 then
        -- ADDICTION is an array - add all tokens
        for _, t in ipairs(tok) do
          table.insert(out, t)
        end
      elseif tag ~= TAG_STATUS_ADDICTION or type(tok) ~= "table" then
        -- Regular status: single token (or ADDICTION that hasn't been converted to array yet)
...
function TE_RefreshStatuses(color)
  ...
  placeTokensSequential(board, buildOrderedStatusTokens(color), slots, STEP_DELAY)
end
...
function TE_HasStatus(color, statusTag)
  ...
  -- Check for ADDICTION (can be array) vs regular status (single token)
  local tok = s.active[statusTag]
  if not tok then return false end
  
  if statusTag == TAG_STATUS_ADDICTION then
    -- ADDICTION can be array - check if it has any tokens
...
function TE_AddStatus(color, statusTag)
  ...
  -- ADDICTION supports multiple tokens (array), other statuses are single token
  if statusTag == TAG_STATUS_ADDICTION then
    -- ADDICTION: allow multiple tokens
    if not s.active[statusTag] then
      s.active[statusTag] = {}  -- Initialize as array
    elseif type(s.active[statusTag]) ~= "table" or #s.active[statusTag] == 0 then
      -- Convert existing single token to array (backward compatibility)
      local oldTok = s.active[statusTag]
      s.active[statusTag] = {}
      if oldTok and type(oldTok) ~= "table" then
    ...
    if activeCount >= 6 then
      print("[WLB_TOKEN_ENGINE] ERROR: Max 6 statuses already active for "..tostring(color))
      return
    end
...
function TE_RemoveStatus(color, statusTag)
  ...
  -- ADDICTION supports multiple tokens (array), remove one
  if statusTag == TAG_STATUS_ADDICTION and type(tok) == "table" and #tok > 0 then
    ...
    if type(tok) == "table" then
      -- Handle case where ADDICTION was converted but RemoveStatus called with old token
...
function TE_ClearStatuses(color)
  ...
  for tag, tok in pairs(s.active) do
    -- Handle ADDICTION array or regular single token
    if tag == TAG_STATUS_ADDICTION and type(tok) == "table" then
      -- ADDICTION is an array - recycle all tokens
      for _, t in ipairs(tok) do
...
function onLoad()
  ...
  -- Row 4 (Status add)
  local sx = {-1.70,-1.02,-0.34,0.34,1.02,1.70}
  local sLabs = {"+GK","+EXP","+DATE","+SICK","+WND","+ADD"}
  local sFns  = {"btnAddGK","btnAddEXP","btnAddDATE","btnAddSICK","btnAddWND","btnAddADD"}
  for i=1,6 do
    makeBtn(sLabs[i], sFns[i], sx[i], 0.55, 300, 200, 120, {0.20,0.20,0.20})
  end

  -- Row 5 (Status remove + clear)
  local rx = {-1.70,-1.02,-0.34,0.34,1.02,1.70, 2.35}
  local rLabs = {"-GK","-EXP","-DATE","-SICK","-WND","-ADD","CLEAR"}
  local rFns  = {"btnRemGK","btnRemEXP","btnRemDATE","btnRemSICK","btnRemWND","btnRemADD","btnClearStatuses"}
  for i=1,7 do
    local btnColor = (i==7) and {0.45,0.12,0.12} or {0.12,0.12,0.12}
    local btnWidth = (i==7) and 360 or 300
